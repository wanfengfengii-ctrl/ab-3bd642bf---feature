...
纯前端应用：修复师在本地工作台为每个**方案**建立槽位、器物初始归属、目标上限（电导率，整数 µS/cm）与所需连续轮数；启动后逐轮提交覆盖该槽全部仍在浸泡器物的整数读数，系统据此判定**换液**与**出槽**资格，避免把单件短暂下降误作整槽合格。已提交的整轮读数若被抄错，可在槽位过程页提出**追溯补正**，仅当补正不推翻既有处置依据时才会被保存。
...
- **追溯补正**：对已提交的整轮读数可提出替代读数——只替换原轮每件器物的非负整数读数，器物集合与采样时刻完全保持不变，同一原轮至多一条有效补正。提交时替代读数按原轮位置叠入并自首项记录重放，重新验证其后每次读数、换液与出槽；任一事件失去依据即拒绝写入，并报告最早受影响事件及原因。补正成功后原读数与补正均在过程记录中可见，当前趋势、资格与修订号以补正后的重放结果一致还原。
...
src/
  domain/             纯领域逻辑（可在 Node 中测试）
    ...
    correction.js     追溯补正：结构校验、叠入重放与全流重新验证
  store/
